<?php
// old, unfinished and not used anywhere
...
// /**
//   * Provides a Resource to get and patch SciTalk collections
//   *
//   * @RestResource(
//   *   id = "scitalk_talk_collection",
//   *   label = @Translation("SciTalk Talk REST API - Collection"),
//   *   entity_type = "taxonomy_term",
//   *   serialization_class = "Drupal\taxonomy\Entity\Term",
//   *   uri_paths = {
//   *     "canonical" = "/api/talk/collection/{collection_id}",
//   *     "https://www.drupal.org/link-relations/create" = "/api/talk/collection"
//   *   }
//   * )
//   */

// class SciTalkResourceCollection extends ResourceBase {
...
//   protected const VOCABULARY_NAME = 'collection';
...
//     if ($collection_id == 'all') return $this->getAll();

//     $taxonomy_query = \Drupal::entityQuery('taxonomy_term')
//       ->condition('vid', SciTalkResourceCollection::VOCABULARY_NAME)
//       ->condition('name', $collection_id, '=')
//       ->accessCheck(TRUE)
//       ->range(0,1)
//       ->execute();
...
//     }
//     else {
//       return new ResourceResponse('Collection not found: ' . $collection_id, 200);
//       //throw new BadRequestHttpException('Invalid Series Number received: ' . $collection_id);
//     }
...
//     }
//     else {
//       return new ResourceResponse('No collections found.', 200);
...
//     if ($data->bundle() != SciTalkResourceCollection::VOCABULARY_NAME) {
...
//     $collection_id = $this->idGenerator->generateReferenceId($data);

//     //add it to the data:
//     $data->set('field_collection_number', $collection_id);
//     $data->set('path', '/'.$collection_id);

//     /* 
//     //i can either do as below or just get the data sent to save it.
//     //need to see what's best

//     $data->set('name', $data->name->value);
//     $data->set('description', $data->description->value ?? '');
//     $data->set('field_collection_short_desc', $data->field_collection_short_desc->value ?? '');

//     if (!empty($data->field_collection_event_url))
//     {
//       $data->set('field_collection_event_url', ["uri" => $data->field_collection_event_url->uri, 
//                                                 "title" => $data->field_collection_event_url->title, 
//                                                 "options" => ["target" => "_blank"]
//                                               ]
//                 );
//     }
    
//     if (!empty($data->field_collection_start_date))
//     {
//       $data->set('field_collection_start_date', $data->field_collection_start_date->value);
//     }       
    
//     if (!empty($data->field_collection_end_date))
//     {
//       $data->set('field_collection_end_date', $data->field_collection_end_date->value);
//     }       

//     if (!empty($data->field_collection_type))
//     {
//       $data->set('field_collection_type', $data->field_collection_type->target_id);
//     }
//   */  
...
//     try {
//       $entity->save();
//       $this->logger->notice('Created Collection %type with ID %id and collection Id %collection_id.', ['%type' => $entity->getEntityTypeId(), '%id' => $entity->id(), '%collection_id' => $entity->name->value]);
...
//    * Throws exception expected.
//    */
//   public function patch($collection_id, $data, $request) {
//     if (empty($collection_id)) {
//       throw new BadRequestHttpException('No value for collection number received.');
//     }
...
//         $this->logger->notice('Updated entity %type with ID %id and Series number %collection_id.', ['%type' => $original_entity->getEntityTypeId(), '%id' => $original_entity->id(), '%collection_id' => $original_entity->name->value]);

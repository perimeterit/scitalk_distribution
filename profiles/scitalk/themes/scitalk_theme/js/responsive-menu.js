(function ($) {
  ...
  Drupal.behaviors.scitalk_mobile_menus = {
   attach: function (context, settings) {
     ...
     // Take out the DOM rearranging. We don't need it now. Instead just use CSS
     // And JS is only needed to toggle it on and off.
     // Build the menu if the browser with is smaller than the breakpoint
      $('.left-nav-wrapper', context).prepend('<button class="menu--close menu--icon" aria-expanded="true"><span class="visually-hidden">' + $('.menu-toggle span').html() +  '</span></button>');
